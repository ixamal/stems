...
.venv/bin/python -m py.exec.separate --path ~/Music/stems_audio --execute   # remaining crate
# After that job exits — drop leftover mute/acappella pairs, no GPU:
.venv/bin/python -m py.exec.separate --path ~/Music/stems_audio --drop-pairs-only
.venv/bin/python -m py.exec.separate --path ~/Music/stems_audio --drop-pairs-only --execute
```
